...
describe('PostgreSQL transactions', () => {
  ...
  it('commits writes when the transaction succeeds', async () => {
    ...
    await expect(DbUser.query().count()).resolves.toBe(3)
    await expect(
      DbUser.query().whereKey('email', 'mia-transaction@example.com').exists(),
    ).resolves.toBe(true)
  })

  it('rolls back writes when the transaction callback throws', async () => {
    await expect(
      DbUser.transaction(async () => {
        await DbUser.query().create({
          name: 'Rollback',
    ...
    await expect(
      DbUser.query().whereKey('email', 'rollback-transaction@example.com').exists(),
    ).resolves.toBe(false)
  })
  ...
  it('reuses the active transaction for nested transaction calls', async () => {
    ...
    await expect(DbUser.query().count()).resolves.toBe(2)
    await expect(
      DbUser.query().whereKey('email', 'outer-transaction@example.com').exists(),
    ).resolves.toBe(false)
    await expect(
      DbUser.query().whereKey('email', 'inner-transaction@example.com').exists(),
    ).resolves.toBe(false)
  })
})
