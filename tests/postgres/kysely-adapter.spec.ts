...
describe('PostgreSQL Kysely adapter', () => {
  ...
  const db = new Kysely<Record<string, never>>({
    ...
    log(event) {
      if (event.level === 'query') executedQueries.push(event.query.sql)
    },
  })
  ...
  it('matches Prisma adapter CRUD behavior for core specs', async () => {
    ...
    if (typeof insertedId !== 'number') throw new Error('Expected inserted id to be a number.')
  ...
  it('supports core QueryBuilder CRUD and pagination against Postgres', async () => {
    ...
    await expect(DbUser.query().whereKey('email', 'mia-kysely@example.com').exists()).resolves.toBe(
      true,
    )
    ...
    expect(trashedArticles.all().map((article) => article.getAttribute('title'))).toEqual([
      'Archived',
    ])
  })
  ...
  it('supports raw and aliased select expressions through QueryBuilder', async () => {
    ...
    const rawExpression = await DbUser.query().select('1 as "isActive"').firstOrFail()
  ...
  it('supports raw where clauses through the Kysely adapter', async () => {
    ...
    const query = DbUser.query().whereRaw('LOWER("email") = ? OR LOWER("email") LIKE ?', [
  ...
  it('auto-quotes bare camelCase identifiers in raw where clauses', () => {
    ...
    const inspection = DbUser.query()
      .whereRaw('createdAt < ? and users.updatedAt is not null and status = ?', [
        new Date('2020-01-01'),
        'active',
      ])
      .inspect()
  ...
  it('compiles the join family into SQL', () => {
    ...
    const compoundSql = DbUser.query()
      .join('posts', (join) =>
        join.on('users.id', 'posts.userId').where('posts.published', '=', true),
      )
      .inspect()
      ?.sql?.replace(/\s+/g, ' ')
    expect(compoundSql).toContain(
      'inner join "posts" on "users"."id" = "posts"."userId" and "posts"."published" = $1',
    )

    const crossSql = DbUser.query().crossJoin('posts').inspect()?.sql?.replace(/\s+/g, ' ')
  ...
  it('compiles subquery and lateral joins into SQL', () => {
    ...
    const subSql = DbUser.query()
      ...
      .inspect()
      ?.sql?.replace(/\s+/g, ' ')
    expect(subSql).toContain(
      'inner join ( select * from "posts" where "userId" = $1 ) as "p" on "users"."id" = "p"."userId"',
    )
  ...
  it('supports SQL-backed direct relation filters and aggregates through QueryBuilder', async () => {
    ...
    const whereHasA = await DbUser.query()
      .whereHas('posts', (query) => query.where({ title: 'A' }))
      .get()
  ...
  it('executes eager loading through the Kysely adapter relationLoads path', async () => {
    ...
    const users = await DbUser.query().with('posts.comments').orderBy({ id: 'asc' }).get()

    expect(loadRelationsSpy).toHaveBeenCalledTimes(2)
    expect(loadRelationsSpy).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({
        relations: [
          {
            relation: 'posts',
            relationLoads: [{ relation: 'comments', relationLoads: undefined }],
          },
        ],
      }),
    )
    expect(loadRelationsSpy).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({
        relations: [{ relation: 'comments', relationLoads: undefined }],
      }),
    )
    ...
    const firstPostComments = firstUserPosts
      .all()[0]
      ?.getAttribute('comments') as ArkormCollection<unknown>
  ...
  it('supports constrained eager loading and model.load() through Arkorm relation load specs on the Kysely path', async () => {
    ...
    expect(
      (posts.all()[0]?.getAttribute('comments') as ArkormCollection<unknown>).all(),
    ).toHaveLength(1)
    ...
    if (!reloaded) throw new Error('Expected reloaded user to exist.')
    ...
    expect(
      (reloadedPosts.all()[0]?.getAttribute('comments') as ArkormCollection<unknown>).all(),
    ).toHaveLength(1)
    ...
    expect(normalizedSql).toContain(
      'select * from "posts" where ("userId" in ($1) and "title" = $2) order by "id" desc limit $3',
    )
    expect(normalizedSql).toContain(
      'select * from "comments" where ("commentableId" in ($1) and "commentableType" = $2)',
    )
  })
  ...
  it('matches Prisma compatibility and Kysely adapter behavior for adapter-first relation filters and eager loading without delegate warnings', async () => {
    ...
    const runScenario = async (adapter: DatabaseAdapter) => {
      ...
      return {
        ...
        roleNames: roles
          .all()
          .map((role) => role.getAttribute('name'))
          .sort(),
      }
    }
  ...
  it('supports SQL-backed belongsToMany relation filters and aggregates through QueryBuilder', async () => {
    ...
    const whereHasAdminRole = await DbUser.query()
      .whereHas('roles', (query) => query.where({ name: 'admin' }))
      .get()
    expect(whereHasAdminRole.all().map((user) => user.getAttribute('id'))).toEqual([1])

    const withoutAdminRole = await DbUser.query()
      .whereDoesntHave('roles', (query) => query.where({ name: 'admin' }))
      .get()
  ...
  it('supports belongsToMany write helpers through the Kysely adapter', async () => {
    ...
    expect(finalRoles.all().map((role) => role.getAttribute('name'))).toEqual([
      'admin',
      'reviewer',
      'observer',
    ])
  ...
  it('supports SQL-backed through relation filters and aggregates through QueryBuilder', async () => {
    ...
    const hasPostImages = await DbUser.query()
      .has('postImages', '>=', 2)
      .orderBy({ id: 'asc' })
      .get()
    expect(hasPostImages.all().map((user) => user.getAttribute('id'))).toEqual([1])

    const whereHasImageA = await DbUser.query()
      .whereHas('postImages', (query) => query.where({ url: 'a.png' }))
      .get()
    ...
    const withoutAvatarA = await DbUser.query()
      .whereDoesntHave('avatar', (query) => query.where({ url: 'a.png' }))
      .orderBy({ id: 'asc' })
      .get()
  ...
  it('supports SQL-backed OR and negative relation helper variants through QueryBuilder', async () => {
    ...
    const orHasPosts = await DbUser.query()
      .whereKey('id', 2)
      .orHas('posts', '>=', 2)
      .orderBy({ id: 'asc' })
      .get()
    ...
    const orWhereHasPosts = await DbUser.query()
      .whereKey('id', 2)
      .orWhereHas('posts', (query) => query.where({ title: 'A' }))
      .orderBy({ id: 'asc' })
      .get()
    expect(orWhereHasPosts.all().map((user) => user.getAttribute('id'))).toEqual([1, 2])

    const orDoesntHaveAvatar = await DbUser.query()
      .whereKey('id', 1)
      .orDoesntHave('avatar')
      .orderBy({ id: 'asc' })
      .get()
    ...
    const orWhereDoesntHaveAvatar = await DbUser.query()
      .whereKey('id', 1)
      .orWhereDoesntHave('avatar', (query) => query.where({ url: 'a.png' }))
      .orderBy({ id: 'asc' })
      .get()
  ...
  it('falls back for unsupported relation helpers while preserving correctness, aggregates, and pagination semantics', async () => {
    ...
    const orHasComments = await DbUser.query()
      .whereKey('id', 2)
      .orHas('comments')
      .orderBy({ id: 'asc' })
      .get()
    ...
    const orWhereHasComments = await DbUser.query()
      .whereKey('id', 2)
      .orWhereHas('comments', (query) => query.where({ body: 'Hi user' }))
      .orderBy({ id: 'asc' })
      .get()
    expect(orWhereHasComments.all().map((user) => user.getAttribute('id'))).toEqual([1, 2])

    const orDoesntHaveComments = await DbUser.query()
      .whereKey('id', 1)
    ...
    const orWhereDoesntHaveComments = await DbUser.query()
      .whereKey('id', 1)
      .orWhereDoesntHave('comments', (query) => query.where({ body: 'Hi user' }))
      .orderBy({ id: 'asc' })
      .get()
  ...
  it('supports SQL-backed conflict-handling write helpers through QueryBuilder', async () => {
    ...
    await expect(DbUser.query().where({ email: 'john@example.com' }).value('name')).resolves.toBe(
      'John Conflict Updated',
    )
    await expect(DbUser.query().where({ email: 'jane@example.com' }).value('name')).resolves.toBe(
      'Jane Conflict Updated',
    )
  ...
  it('falls back for updateOrInsert when conflict keys are not backed by a unique constraint', async () => {
    ...
    await expect(DbUser.query().where({ name: 'Casey Fallback' }).value('email')).resolves.toBe(
  ...
  it('uses RETURNING-aware single-row update and bulk delete for non-unique QueryBuilder writes', async () => {
    ...
    const updated = await DbUser.query()
      .where({ isActive: 1 })
      .update({ name: 'First Active Updated' })
    ...
    expect(activeUsers.all().map((user) => user.getAttribute('name'))).toContain(
      'First Active Updated',
    )
  ...
  it('wraps invalid column errors with compiled SQL context', async () => {
    const error = await kyselyAdapter
      .select({
        target: { table: 'users' },
        where: {
          type: 'comparison',
  ...
  it('runs adapter transactions against Postgres', async () => {
    ...
    await expect(
      kyselyAdapter.transaction(async (transactionAdapter) => {
        await transactionAdapter.insert({
          target: { table: 'users', primaryKey: 'id' },
          values: {
            name: 'Rollback User',
