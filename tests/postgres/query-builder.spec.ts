...
describe('PostgreSQL QueryBuilder', () => {
  ...
  it('supports filtering parity helpers', async () => {
    const orWhere = await DbUser.query().whereKey('id', 99999).orWhere({ id: 2 }).get()
    expect(orWhere.all().map((user) => user.getAttribute('id'))).toEqual([2])

    const whereNot = await DbUser.query()
      .whereNot({ isActive: 1 } as Record<string, unknown>)
      .get()
    ...
    const firstWhereComparison = await DbUser.query()
      .orderBy({ id: 'asc' })
      .firstWhere('id', '>', 1)
    ...
    const orWhereNotIn = await DbUser.query()
      .whereKey('id', 1)
      .orWhereNotIn('id', [1])
      .orderBy({ id: 'asc' })
      .get()
    ...
    const whereDate = await DbArticle.query()
      .withTrashed()
      .whereDate('deletedAt', '2026-03-04')
      .get()
  ...
  it('supports read helpers and utility shortcuts', async () => {
    ...
    await expect(DbUser.query().whereKey('id', 99999).valueOrFail('email')).rejects.toThrow(
      'Record not found.',
    )
  ...
  it('supports aggregate and advanced query helpers', async () => {
    ...
    expect(() => DbUser.query().whereRaw('id = ?', [1])).toThrow(
      'Raw where clauses are not supported by the current adapter.',
    )
    expect(() => DbUser.query().orWhereRaw('id = ?', [1])).toThrow(
      'Raw where clauses are not supported by the current adapter.',
    )
  })
  ...
  it('supports relationship existence/query helpers', async () => {
    ...
    const orWhereHas = await DbUser.query()
      .whereKey('id', 2)
  ...
  it('supports insert and upsert family write helpers', async () => {
    ...
    const updatedCount = await DbUser.query()
  ...
  it('throws firstOrFail when no records match', async () => {
    await expect(DbUser.query().whereKey('id', 99999).firstOrFail()).rejects.toThrow(
      'Record not found.',
    )
  })
  ...
  it('throws for update/delete without where constraints', async () => {
    await expect(DbUser.query().update({ name: 'Nope' })).rejects.toThrow(
