...
describe('migration history helpers', () => {
  ...
  it('resolves default and custom state file paths', () => {
    ...
    expect(resolveMigrationStateFilePath(cwd, './database/custom-state.json')).toContain(
      '/database/custom-state.json',
    )
  })

  it('builds stable migration identity from file and class', () => {
    const identity = buildMigrationIdentity(
      '/workspace/database/migrations/20260312102000_create_users.ts',
