...
describe('where helpers', () => {
  ...
  describe('LIKE family (portable)', () => {
    ...
    it('orWhereLike combines with OR semantics', async () => {
      const users = await User.query().where({ isActive: 1 }).orWhereLike('email', 'john').get()
  ...
  describe('PostgreSQL-only predicates reject on the Prisma compatibility adapter', () => {
    it('rejects whereJsonContains', async () => {
      await expect(
        User.query().whereJsonContains('meta', { tier: 'pro' }).get(),
      ).rejects.toBeInstanceOf(UnsupportedAdapterFeatureException)
    })

    it('rejects whereJsonContainsKey', async () => {
      await expect(User.query().whereJsonContainsKey('meta->tier').get()).rejects.toBeInstanceOf(
        UnsupportedAdapterFeatureException,
      )
    })

    it('rejects whereJsonLength', async () => {
      await expect(User.query().whereJsonLength('meta', '>', 1).get()).rejects.toBeInstanceOf(
        UnsupportedAdapterFeatureException,
      )
    })

    it('rejects whereJsonOverlaps', async () => {
      await expect(User.query().whereJsonOverlaps('meta', ['a']).get()).rejects.toBeInstanceOf(
        UnsupportedAdapterFeatureException,
      )
    })
    ...
    it('rejects having', async () => {
      await expect(
        User.query().groupBy('isActive').having('isActive', '>', 1).get(),
      ).rejects.toBeInstanceOf(UnsupportedAdapterFeatureException)
    })
  })
