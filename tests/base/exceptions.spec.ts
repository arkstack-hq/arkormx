...
describe('Exceptions', () => {
  ...
  it('throws MissingDelegateException with delegate resolution context', async () => {
    ...
    const thrown = await Ghost.query()
      .get()
      .catch((error) => error)
  ...
  it('throws QueryConstraintException for invalid update and insertUsing constraints', async () => {
    await expect(User.query().updateFrom({ name: 'Jane' } as never)).rejects.toBeInstanceOf(
      QueryConstraintException,
    )
    await expect(User.query().insertUsing(['name'], null)).rejects.toBeInstanceOf(
      QueryConstraintException,
    )
  })
  ...
  it('throws ModelNotFoundException and UniqueConstraintResolutionException with operation context', async () => {
    await expect(User.query().whereKey('id', 999).firstOrFail()).rejects.toBeInstanceOf(
      ModelNotFoundException,
    )
    await expect(
      User.query().insertGetId({ name: 'Jane' } as never, 'uuid'),
    ).rejects.toBeInstanceOf(UniqueConstraintResolutionException)
  })

  it('throws RelationResolutionException when relationship resolution fails', async () => {
    await expect(User.query().has('missingRelation').get()).rejects.toBeInstanceOf(
      RelationResolutionException,
    )
  })
})
