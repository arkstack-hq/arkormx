import { SchemaBuilder, UnsupportedAdapterFeatureException, configureArkormRuntime, resetArkormRuntimeForTests, type DatabaseAdapter } from '../../src'
...
describe('SchemaBuilder foreign-key constraint toggles', () => {
    ...
    it('withoutForeignKeyConstraints re-enables constraints even when the callback throws', async () => {
        ...
        await expect(SchemaBuilder.withoutForeignKeyConstraints(async () => {
    ...
    it('throws when the adapter does not support raw queries', async () => {
        ...
        await expect(SchemaBuilder.disableForeignKeyConstraints())
