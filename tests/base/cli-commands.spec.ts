...
} from '../../src/helpers/migration-history'
...
describe('CLI command classes', () => {
  it('MakeFactoryCommand creates a factory file', async () => {
    ...
    const { successLines, errorLines } = attachCommandIo(
  ...
  it('MakeMigrationCommand creates a migration file', async () => {
    ...
    const generatedMigrationFile = readdirSync(migrationsDirectory).find((file) =>
      file.endsWith('.js'),
    )
    expect(generatedMigrationFile).toBeTruthy()

    const generatedFileSource = readFileSync(
      join(migrationsDirectory, generatedMigrationFile as string),
      'utf-8',
    )
  ...
  it('InitCommand creates arkormx.config.js from configured stub path', async () => {
    ...
    expect(errorLines).toHaveLength(0)
    expect(successLines.some((line) => line.includes('Arkormˣ initialized successfully'))).toBe(
      true,
    )
  })
  ...
  it('ModelsSyncCommand includes json, enum imports, and list declarations', async () => {
    ...
    expect(updatedSource).toContain(
      'declare preferences: Record<string, unknown> | unknown[] | null',
    )
  ...
  it('ModelsSyncCommand prefers persisted mapped attribute names for adapter introspection results', async () => {
    ...
    const updatedSource = readFileSync(userModelPath, 'utf-8')
    expect(updatedSource).toContain(
      "declare accountType: 'personal' | 'family' | 'business' | null",
    )
  ...
  it('MigrationHistoryCommand shows and resets tracked migration state', async () => {
    ...
    expect(
      verifyIo.successLines.some((line) => line.includes('No tracked migrations found.')),
    ).toBe(true)
  })
  ...
  it('MigrateRollbackCommand rolls back last run by default and honors --step', async () => {
    ...
    writeFileSync(
      join(migrationsDir, 'CreatePostsMigration.ts'),
      [
        `import { Migration } from '${migrationBaseImport}'`,
        '',
    ...
    expect(
      rollbackDryRunIo.successLines.some((line) =>
        line.includes('Dry run: 2 migration(s) would be rolled back.'),
      ),
    ).toBe(true)
    ...
    const stateAfterDryRun = readAppliedMigrationsState(
      join(workspace, '.arkormx', 'migrations.applied.json'),
    )
    ...
    expect(
      rollbackLastRunIo.successLines.some((line) => line.includes('Rolled back 2 migration(s).')),
    ).toBe(true)
    ...
    const migrateUsersIo = attachCommandIo(
      migrateUsers as unknown as any,
      {
        'skip-generate': true,
        'skip-migrate': true,
        schema: schemaPath,
    ...
    const migratePostsIo = attachCommandIo(
      migratePosts as unknown as any,
      {
        'skip-generate': true,
        'skip-migrate': true,
        schema: schemaPath,
    ...
    expect(
      rollbackStepIo.successLines.some((line) => line.includes('Rolled back 1 migration(s).')),
    ).toBe(true)
  ...
  it('MigrateFreshCommand rebuilds schema and tracked state from a clean baseline', async () => {
    ...
    expect(
      freshIo.successLines.some((line) => line.includes('Refreshed database with 1 migration(s).')),
    ).toBe(true)
    ...
    const stateAfterFresh = readAppliedMigrationsState(
      join(workspace, '.arkormx', 'migrations.applied.json'),
    )
  ...
  it('MigrateCommand loads migrations from registered extra paths', async () => {
    ...
    const migrationBaseImport = `${originalCwd.replace(/\\/g, '/')}/src/database/Migration.ts`
    writeFileSync(
      join(packageMigrationsDir, 'CreatePluginUsersMigration.ts'),
      [
        `import { Migration } from '${migrationBaseImport}'`,
        '',
  ...
  it('SeedCommand runs explicitly registered seeders by name', async () => {
    ...
    const seedIo = attachCommandIo(
      seedCommand as unknown as any,
      {},
      {
  ...
  it('MigrateFreshCommand runs prisma generate and db push when skip flags are not set', async () => {
    ...
    const prismaCommandSpy = vi
      .spyOn(migrationHelpers, 'runPrismaCommand')
    ...
    try {
      ...
      expect(freshIo.errorLines).toHaveLength(0)
      expect(prismaCommandSpy).toHaveBeenNthCalledWith(
        1,
      ...
      expect(prismaCommandSpy).toHaveBeenNthCalledWith(
        2,
        ['db', 'push', '--force-reset', '--schema', schemaPath],
        currentWorkspace,
      )
    } finally {
  ...
  it('MigrateFreshCommand reports when no migration classes are available', async () => {
    ...
    expect(io.successLines).toHaveLength(0)
    expect(io.errorLines.some((line) => line.includes('No migration classes found to run.'))).toBe(
      true,
    )
  })
