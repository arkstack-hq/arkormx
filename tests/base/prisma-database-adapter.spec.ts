...
describe('Prisma database adapter', () => {
  ...
  it('wraps raw delegate failures in QueryExecutionException', async () => {
    ...
    if (!error) throw new Error('Expected adapter.select() to throw QueryExecutionException.')
  ...
  it('isolates compatibility-only Prisma adapter gaps behind explicit capability flags and unsupported-feature errors', async () => {
    ...
    await expect(
      ...
      message: expect.stringContaining(
        'Adapter-owned relation batch loading is intentionally unavailable on the Prisma compatibility adapter',
      ),
    })
