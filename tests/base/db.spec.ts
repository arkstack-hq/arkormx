...
describe('DB facade', () => {
  it('supports direct raw queries without setting a table target', async () => {
    ...
    const rows = await DB.raw<{ id: number; name: string }>('select * from users where name = ?', [
      'Jane',
    ])
  ...
  it('resolves updates using custom table metadata without a model', async () => {
    ...
    expect(selectOneSpecs[0]).toEqual(
      expect.objectContaining({
        target: expect.objectContaining({
          table: 'users',
          primaryKey: 'uuid',
  ...
  it('supports raw table insert metadata for generated keys and timestamps', async () => {
    ...
    const created = await DB.table<{ id: string; createdAt: Date; updatedAt: Date; name: string }>(
      'api_tokens',
      {
      ...
      JSON.stringify(
        {
          ...
          tables: {
            api_tokens: {
              ...
              timestampColumns: [{ column: 'createdAt', default: 'now()' }],
            },
          },
        },
        null,
