  ...
  return Object.entries(where).every(([key, value]) => {
    ...
    if (value && typeof value === 'object' && !Array.isArray(value)) {
      ...
      if ('in' in clause) {
        ...
        if (!Array.isArray(candidate) || !candidate.includes(rowValue)) return false
      }
      ...
      if ('not' in clause) {
        if (rowValue === clause.not) return false
      }
...
function makeDelegate(rows: Row[]) {
  ...
  return {
    ...
    update: async ({ where, data: payload }: { where: Row; data: Row }) => {
      ...
      if (index < 0) throw new ModelNotFoundException('TestModel', 'Record not found')
