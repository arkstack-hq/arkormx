...
describe('QueryBuilder', () => {
  ...
  it('normalizes raw and aliased select expressions for capable adapters', async () => {
    ...
    try {
      ...
      await User.query().addSelect({ id: true }).addSelect({ '1': 'isActive' }).get()
      expect(select).toHaveBeenLastCalledWith(
        expect.objectContaining({
          columns: [
      ...
      await User.query().select({ isActive: true }).distinct().groupBy('isActive').get()
      expect(select).toHaveBeenLastCalledWith(
        expect.objectContaining({
          distinct: true,
          groupBy: ['isActive'],
  ...
  it('rejects raw select expressions on Prisma compatibility', async () => {
    await expect(User.query().select({ '1': 'isActive' }).get()).rejects.toMatchObject({
      constructor: UnsupportedAdapterFeatureException,
    ...
    await expect(
      User.query().select({ id: true }).addSelect({ '1': 'isActive' }).get(),
  ...
  it('routes core write execution through the configured adapter seam', async () => {
    ...
    try {
      ...
      await User.query()
        .where({ email: 'adapter-create@example.com' })
        .update({ name: 'Adapter Updated Again' })
  ...
  it('supports whereKey and whereIn helpers', async () => {
    const users = await User.query().whereKey('isActive', 1).whereIn('id', [1, 2]).get()
  ...
  it('supports filtering parity helpers', async () => {
    const orWhere = await User.query().whereKey('id', 999).orWhere({ id: 2 }).get()
    ...
    const orWhereNotIn = await User.query()
      .whereKey('id', 1)
      .orWhereNotIn('id', [1])
      .orderBy({ id: 'asc' })
      .get()
    ...
    const whereLike = await User.query()
    ...
    const whereEndsWith = await User.query()
  ...
  it('supports read helpers and utility shortcuts', async () => {
    ...
    await expect(User.query().whereKey('id', 999).valueOrFail('email')).rejects.toThrow(
      'Record not found.',
    )
    ...
    const whenResult = User.query()
      .when(true, (query) => query.whereKey('id', 1))
      .get()
    await expect(whenResult).resolves.toBeInstanceOf(ArkormCollection)

    const unlessResult = User.query()
      .unless(false, (query) => query.whereKey('id', 1))
      .get()
  ...
  it('reports SQL-only structured where helpers on Prisma compatibility', async () => {
    await expect(User.query().whereTime('createdAt', '12:00').get()).rejects.toThrow(
      'Where condition [time] is not supported',
    )
    await expect(User.query().whereDay('createdAt', 4).get()).rejects.toThrow(
      'Where condition [day] is not supported',
    )
    await expect(User.query().whereColumn('name', 'email').get()).rejects.toThrow(
      'Where condition [column-comparison] is not supported',
    )
    await expect(User.query().whereFullText('name', 'Jane').get()).rejects.toThrow(
      'Where condition [full-text] is not supported',
    )
    await expect(User.query().whereExists(User.query().whereKey('id', 1)).get()).rejects.toThrow(
      'Where condition [exists] is not supported',
    )
  })
  ...
  it('supports aggregate and advanced query helpers', async () => {
    ...
    expect(() => User.query().whereRaw('id = ?', [1])).toThrow(
      'Raw where clauses are not supported by the current adapter.',
    )
    expect(() => User.query().orWhereRaw('id = ?', [1])).toThrow(
      'Raw where clauses are not supported by the current adapter.',
    )
  })
  ...
  it('supports relationship existence/query helpers', async () => {
    ...
    const whereDoesntHaveA = await User.query()
      .whereDoesntHave('posts', (query) => query.where({ title: 'A' }))
      .get()
    expect(whereDoesntHaveA.all().map((user) => user.getAttribute('id'))).toEqual([2])

    const orWhereDoesntHaveA = await User.query()
      .whereKey('id', 1)
      .orWhereDoesntHave('posts', (query) => query.where({ title: 'A' }))
      .orderBy({ id: 'asc' })
      .get()
    expect(orWhereDoesntHaveA.all().map((user) => user.getAttribute('id'))).toEqual([1, 2])

    const withCounts = await User.query()
      .withCount('posts')
      .withExists('profile')
      .orderBy({ id: 'asc' })
      .get()
  ...
  it('does not silently fall back for un-compilable relation filters on SQL-capable adapters', async () => {
    ...
    try {
      await expect(
        User.query()
          .whereHas('posts', (query) => query.limit(1))
          .get(),
  ...
  it('supports insert and upsert family write helpers', async () => {
    ...
    expect(total).toBe(9)
    await expect(User.query().where({ email: 'jane@example.com' }).value('name')).resolves.toBe(
      'Jane Upserted',
    )
  })

  it('throws for firstOrFail when no record matches', async () => {
    await expect(User.query().whereKey('id', 999).firstOrFail()).rejects.toThrow(
      'Record not found.',
    )
  })
  ...
  it('deleteOrFail preserves the throwing delete contract', async () => {
    await expect(User.query().where({ id: 1 }).whereNot({ id: 1 }).deleteOrFail()).rejects.toThrow(
      'Record not found for delete operation.',
    )
  })

  it('throws when update or delete are called without where constraints', async () => {
    await expect(User.query().update({ name: 'Nope' })).rejects.toThrow(
