import { LengthAwarePaginator, Paginator, configureArkormRuntime } from '../../src'
...
describe('Pagination', () => {
  ...
  it('normalizes invalid page and perPage values', async () => {
    const page = await User.query().orderBy({ id: 'asc' }).paginate(0, 0)
    ...
    const simplePage = await User.query().orderBy({ id: 'asc' }).simplePaginate(0, 0)
  ...
  it('returns null ranges for empty datasets', async () => {
    ...
    const simplePage = await User.query().whereKey('id', 999).simplePaginate(10, 1)
  ...
  it('returns correct boundary metadata on last page', async () => {
    ...
    const simplePage = await User.query().orderBy({ id: 'asc' }).simplePaginate(1, 2)
  ...
  it('builds pagination URLs using path, query, fragment and pageName options', async () => {
    ...
    const simplePage = await User.query()
      .orderBy({ id: 'asc' })
      .simplePaginate(1, 1, {
        path: '/users',
        query: { filter: 'active' },
  ...
  it('allows URL driver override from runtime config', async () => {
    class FrameworkURLDriver {
      public constructor(private readonly options: { pageName?: string } = {}) {}
      ...
      public url(page: number): string {
        return `framework://pagination/${this.getPageName()}/${Math.max(1, page)}`
      }
    }

    configureArkormRuntime(createCoreClient(), {
      pagination: {
        urlDriver: (options) => new FrameworkURLDriver(options),
      },
    })

    const page = await User.query().orderBy({ id: 'asc' }).paginate(1, 1, { pageName: 'p' })
    expect(page.nextPageUrl()).toBe('framework://pagination/p/2')

    const simplePage = await User.query()
      .orderBy({ id: 'asc' })
      .simplePaginate(1, 1, { pageName: 'cursor' })
  ...
  it('resolves the current page from runtime config when page is omitted', async () => {
    ...
    const page = await User.query()
      .orderBy({ id: 'asc' })
      .paginate(1, undefined, { pageName: 'cursor' })
    expect(page.meta.currentPage).toBe(2)
    expect(page.data.all()[0]?.getAttribute('id')).toBe(2)

    const simplePage = await User.query()
      .orderBy({ id: 'asc' })
      .simplePaginate(1, undefined, { pageName: 'cursor' })
    expect(simplePage.meta.currentPage).toBe(2)
    expect(simplePage.data.all()[0]?.getAttribute('id')).toBe(2)
  })
