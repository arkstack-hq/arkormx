...
describe('Model lifecycle and serialization', () => {
  ...
  it('persists attributes mutated by pre-save events', async () => {
    User.saving((model) => {
      if (model.getAttribute('id') == null) model.setAttribute('email', 'generated@example.com')
      else model.setAttribute('name', 'Updated by event')
    })
  ...
  it('supports class-based event listeners via dispatchesEvents', async () => {
    ...
    const created = new DispatchingUser({
      name: 'Mia',
      email: 'mia-dispatch@example.com',
      isActive: 1,
    })
    await created.save()
  ...
  it('supports model comparison and identity helpers', async () => {
    ...
    const sameRecord = new User({
      id: 1,
      name: 'Jane Clone',
      email: 'clone@example.com',
      isActive: 1,
    })
    const differentRecord = new User({
      id: 2,
      name: 'John',
      email: 'john@example.com',
      isActive: 0,
    })
    const sameIdDifferentClass = new OtherUser({
      id: 1,
      name: 'Jane',
