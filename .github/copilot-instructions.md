# Copilot Instructions for Arkorm

## Big picture architecture

- Arkorm is an Eloquent-style ORM facade over adapter-backed delegates; the core flow is `Model` -> `QueryBuilder` -> adapter delegate (`findMany/findFirst/create/update/delete/count`) -> hydrated model instances.
...
- Subclasses set `protected static override delegate` to logical Arkorm delegate names (commonly plural, e.g. `'users'`, `'articles'`).
...
- Use mapping when Prisma delegate names differ from Arkorm names (e.g. `{ users: 'user', articles: 'article' }` in `tests/postgres.spec.ts`).
