...
export class SeedCommand extends Command<CliApp> {
  ...
  async handle() {
    ...
    if (seederDirs.length === 0 && getRegisteredSeeders().length === 0)
      return void this.error(
        `ERROR: Seeders directory not found: ${this.app.formatPathForLog(configuredSeedersDir)}`,
      )
    ...
    for (const SeederClassItem of classes) {
      executedSeeders.push(...(await Seeder.runWithReport(new SeederClassItem() as Seeder)))
    }
  ...
  private async loadAllSeeders(seedersDirs: string[]): Promise<SeederClass[]> {
    const files = seedersDirs.flatMap((seedersDir) =>
    ...
    const classes = await Promise.all(
      files.map(async (file) => await this.loadSeederClassesFromFile(file)),
    )

    return [...classes.flat(), ...(getRegisteredSeeders() as SeederClass[])]
  }
   ...
   * @returns
   */
  private async loadNamedSeeder(seedersDirs: string[], name: string): Promise<SeederClass[]> {
    const base = name.replace(/Seeder$/, '')
    const registered = getRegisteredSeeders().find(
      (cls) => cls.name === name || cls.name === `${base}Seeder`,
    )
    if (registered) return [registered as SeederClass]
  ...
  private async loadSeederClassesFromFile(filePath: string): Promise<SeederClass[]> {
    ...
    return exports.filter((value): value is SeederClass => {
