...
import {
  deletePersistedColumnMappingsState,
  resolveColumnMappingsFilePath,
...
import {
  ...
  supportsDatabaseMigrationState,
...
export class MigrationHistoryCommand extends Command<CliApp> {
  ...
  async handle() {
    ...
    if (this.option('reset')) {
      await writeAppliedMigrationsStateToStore(
        adapter,
        stateFilePath,
        createEmptyAppliedMigrationsState(),
      )
      ...
      .forEach((migration) => {
        this.success(
          this.app.splitLogger(
            'Applied:',
            `${migration.id.split(':').at(0)} @ ${migration.appliedAt}`,
          ),
        )
      })
  }
