...
export class CliApp {
  ...
  resolveRuntimeScriptPath(filePath: string): string {
    ...
    if (typeof buildOutput === 'string' && buildOutput.trim().length > 0) {
      ...
      if (relativeSource && !relativeSource.startsWith('..')) {
        ...
        const mappedIsTs =
          mappedExtension === '.ts' || mappedExtension === '.mts' || mappedExtension === '.cts'
  ...
  public makeFactory(
    ...
    const factoriesDir = this.resolveConfigPath(
      'factories',
      join(process.cwd(), 'database', 'factories'),
    )
    const outputPath = join(factoriesDir, `${factoryName}.${outputExt}`)
    const modelPath = join(
      this.resolveConfigPath('models', join(process.cwd(), 'src', 'models')),
      `${modelName}.${outputExt}`,
    )
    const relativeImport =
      options.modelImportPath ??
  ...
  public makeMigration(name: string): { name: string; path: string } {
    const migrationsDir = this.resolveConfigPath(
      'migrations',
  ...
  public makeModel(
    ...
    const baseName = str(name.replace(/Model$/, ''))
      .pascal()
      .toString()
    ...
    const factoryPath = join(
      this.resolveConfigPath('factories', join(process.cwd(), 'database', 'factories')),
      `${factoryName}.${outputExt}`,
    )
    ...
    const modelPath = this.generateFile(
      stubPath,
      outputPath,
      {
        ModelName: modelName,
  ...
  private ensurePrismaModelEntry(
    ...
    if (existingByTable || existingByName) return { path: schemaPath, updated: false }
  ...
  private parseDeclarationType(value: string): ParsedDeclarationNode | null {
    ...
    if (trimmed === 'Record<string, unknown>') return { kind: 'named', name: trimmed }

    if (/^[A-Za-z_$][A-Za-z0-9_$]*$/.test(trimmed)) return { kind: 'named', name: trimmed }
  ...
  private isDeclarationNodeAssignable(
    ...
    if (expected.kind === 'named') {
      ...
      if (actual.kind === 'string-literal') {
        ...
        if (enumValues?.includes(actual.value)) return true
      }
    ...
    if (expected.kind === 'array')
      return (
        actual.kind === 'array' &&
        this.isDeclarationAssignable(actual.element, expected.element, enums)
      )
  ...
  private syncPrismaEnumImports(modelSource: string, enumTypes: string[]): string {
    ...
    if (existingImport) {
      ...
      const mergedTypes = [...new Set([...existingTypes, ...enumTypes])].sort((left, right) =>
  ...
  private parseModelSyncSource(modelSource: string): SyncedModelSource | null {
    ...
    const tableMatch =
      modelSource.match(/protected\s+static\s+override\s+table\s*=\s*['"]([^'"]+)['"]/) ??
      modelSource.match(/static\s+table\s*=\s*['"]([^'"]+)['"]/)
    const compatibilityDelegateMatch =
      modelSource.match(/protected\s+static\s+override\s+delegate\s*=\s*['"]([^'"]+)['"]/) ??
      modelSource.match(/static\s+delegate\s*=\s*['"]([^'"]+)['"]/)

    return {
      className,
      table:
        tableMatch?.[1] ??
        compatibilityDelegateMatch?.[1] ??
        str(className).camel().plural().toString(),
    }
  }
  ...
  private syncModelRegistryTypes(modelFiles: string[]): { path: string; updated: boolean } {
    ...
    const models = modelFiles
      ...
      .filter((model): model is { className: string; importPath: string } => model !== null)
      .sort((left, right) => left.className.localeCompare(right.className))

    const nameRegistryLines = models.map((model) => {
      return `    ${model.className}: true`
    })
    const getModelOverloads = models.flatMap((model) => [
      '  function getModel(',
    ...
    const current = existsSync(registryPath) ? readFileSync(registryPath, 'utf-8') : null
    if (current === content) return { path: registryPath, updated: false }

    mkdirSync(registryDir, { recursive: true })
    writeFileSync(registryPath, content)

    return { path: registryPath, updated: true }
  }

  private resolveModelRegistryImportPath(registryPath: string, modelPath: string): string {
    const realModelPath = realpathSync(modelPath)
    const withoutExtension = realModelPath.slice(0, -extname(realModelPath).length)
    const relativePath = relative(dirname(registryPath), withoutExtension).replace(/\\/g, '/')
  ...
  private applyPersistedFieldMetadata(structure: AdapterModelStructure): AdapterModelStructure {
    ...
    if (
      Object.keys(persistedMetadata.columns).length === 0 &&
      Object.keys(persistedMetadata.enums).length === 0
    )
      return structure

    const attributesByColumn = Object.entries(persistedMetadata.columns).reduce<
    ...
    return {
      ...
      fields: structure.fields.map((field) => {
        ...
        const enumValues =
          persistedMetadata.enums[logicalName] ?? persistedMetadata.enums[field.name]
  ...
  private syncModelDeclarations(
    ...
    if (classIndex < 0) return { content: modelSource, updated: false }
    ...
    if (classEndIndex < 0) return { content: modelSource, updated: false }
    ...
    const withoutDeclares = withinClass.filter(
      (line) => !/^\s*declare\s+\w+\??:\s*[^\n]+$/.test(line),
    )
    ...
    const enumImports = [
      ...new Set(
        chosenDeclarations.flatMap((declaration) => {
          const type = declaration
            .replace(/^declare\s+\w+\??:\s*/, '')
            .replace(/;$/, '')
            .trim()
  ...
  public async syncModels(
    ...
    if (adapter && typeof adapter.introspectModels === 'function') {
      ...
      const structuresByTable = new Map(
  ...
  public syncModelsFromPrisma(
    ...
    const result = this.syncModelFiles(
      ...
      (filePath, source) => {
        ...
        return (
          prismaModels.find((model) => model.table === parsed.table) ??
          prismaModels.find((model) => model.name === parsed.className)
        )
      },
      prismaEnums,
    )
