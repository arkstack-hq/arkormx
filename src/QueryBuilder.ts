...
export class QueryBuilder<TModel, TDelegate extends ModelQuerySchemaLike = ModelQuerySchemaLike> {
  ...
  private resolvePaginationPage(page: number | undefined, options: PaginationOptions): number {
  ...
  private appendNestedWhere(
    boolean: 'AND' | 'OR',
    callback: WhereCallback<TModel, TDelegate>,
  ): this {
  ...
  private getNestedWhereCondition(): QueryCondition | undefined {
    if (this.legacyWhere) return this.tryBuildQueryCondition(this.legacyWhere) ?? undefined
  ...
  public whereFullText<TKey extends keyof ModelAttributes<TModel> & string>(
    ...
    if (!/^[a-z][a-z0-9_]*$/i.test(language))
      throw new ArkormException(
        'whereFullText() language must be a valid PostgreSQL text search configuration name.',
      )
  ...
  public orWhereFullText<TKey extends keyof ModelAttributes<TModel> & string>(
    ...
    if (!/^[a-z][a-z0-9_]*$/i.test(language))
      throw new ArkormException(
        'orWhereFullText() language must be a valid PostgreSQL text search configuration name.',
      )
  ...
  public async firstWhere(
    ...
    return this.clone()
      .where(this.buildComparisonWhere(key, operator, value))
      .first()
  }
  ...
  private buildComparisonWhere(
    ...
    if (operator === '=') return { [key]: value } as QuerySchemaWhere<TDelegate>

    if (operator === '!=') return { [key]: { not: value } } as QuerySchemaWhere<TDelegate>

    if (operator === '>') return { [key]: { gt: value } } as QuerySchemaWhere<TDelegate>

    if (operator === '>=') return { [key]: { gte: value } } as QuerySchemaWhere<TDelegate>

    if (operator === '<') return { [key]: { lt: value } } as QuerySchemaWhere<TDelegate>
  ...
  public include(include: QuerySchemaInclude<TDelegate>): this {
    ...
    if (normalized === null)
      throw new UnsupportedAdapterFeatureException(
        'Include clauses could not be normalized into Arkorm relation load plans.',
        {
          operation: 'include',
          model: this.model.name,
          meta: {
  ...
  public scope(name: string, ...args: unknown[]): this {
    ...
    if (scoped && scoped !== this) return scoped as this
  ...
  public select(select: QuerySchemaSelect<TDelegate> | ExpressionSelectMap): this {
    ...
    if (normalized === null)
      throw new UnsupportedAdapterFeatureException(
        'Select clauses must use Arkorm-normalizable column projections.',
        {
          operation: 'select',
          model: this.model.name,
        },
  ...
  public addSelect(select: QuerySchemaSelect<TDelegate> | ExpressionSelectMap): this {
    ...
    this.querySelect = [...(this.querySelect ?? [{ column: '*', wildcard: true }]), ...normalized]
  ...
  public crossJoin(table: string, first?: JoinOn): this {
    if (first === undefined) return this.addJoin('cross', table)
  ...
  private buildJoinSubquerySpec(): SelectSpec<TModel> {
    ...
    if (!spec)
      throw new UnsupportedAdapterFeatureException(
        'Subquery join could not be compiled into an Arkorm select specification.',
        {
  ...
  private guardJoinSupport(): void {
    if (!this.adapter?.capabilities?.joins)
      throw new UnsupportedAdapterFeatureException(
        'Join clauses are not supported by the current adapter.',
        {
          operation: 'join',
          model: this.model.name,
          meta: {
  ...
  public inspect(
    ...
    if (operation === 'count') {
      ...
      if (!spec) {
        throw new UnsupportedAdapterFeatureException(
          'Query shape could not be compiled into an Arkorm aggregate specification.',
          {
    ...
    if (!spec) {
      throw new UnsupportedAdapterFeatureException(
        'Query shape could not be compiled into an Arkorm select specification.',
        {
          operation: 'query.inspect',
          model: this.model.name,
        },
    ...
    if (operation === 'select') return adapter.inspectQuery({ operation, spec })
  ...
  public async get(): Promise<ArkormCollection<TModel>> {
    ...
    const useCompatibilityRelationFallback = this.shouldUseCompatibilityRelationFallback(
      useAdapterRelationFeatures,
    )
    const relationCache: RelationResultCache = new WeakMap()
    const rows = await this.executeReadRows()
    const normalizedRows = this.randomOrderEnabled ? this.shuffleRows(rows as unknown[]) : rows
    const models = await this.model.hydrateManyRetrieved(
      normalizedRows as Parameters<ModelStatic<TModel, TDelegate>['hydrateManyRetrieved']>[0],
    )
    ...
    if (this.hasRelationFilters() && useCompatibilityRelationFallback) {
      if (this.hasOrRelationFilters() && this.hasBaseWhereConstraints()) {
        ...
        const allModels = this.model.hydrateMany(
          allRows as Parameters<ModelStatic<TModel, TDelegate>['hydrateMany']>[0],
        )

        filteredModels = await this.filterModelsByRelationConstraints(
          allModels,
          relationCache,
          baseIds,
        )
  ...
  public async first(): Promise<TModel | null> {
    ...
    if (this.randomOrderEnabled) {
      ...
      const model = await this.model.hydrateRetrieved(
        row as Parameters<ModelStatic<TModel, TDelegate>['hydrateRetrieved']>[0],
      )
  ...
  public async insertGetId(
    ...
    const created = (await this.executeInsertRow(
      values as QuerySchemaCreateData<TDelegate>,
    )) as Record<string, unknown>
    const key = sequence ?? this.model.getPrimaryKey()
    if (!(key in created))
      throw new UniqueConstraintResolutionException(
        `Inserted record does not contain key [${key}].`,
        {
   ...
   * @returns
   */
  public async insertUsing(columns: string[], query: unknown): Promise<number> {
   ...
   * @returns
   */
  public async insertOrIgnoreUsing(columns: string[], query: unknown): Promise<number> {
    const rows = await this.resolveInsertUsingRows(columns, query)
    if (rows.length === 0) return 0
  ...
  public async update(data: ModelUpdateData<TModel, TDelegate>): Promise<TModel> {
    ...
    if (
      ...
      directSpec &&
      typeof adapter.updateFirst === 'function'
    ) {
      const updated = await adapter.updateFirst(directSpec)
      if (!updated)
        throw new ModelNotFoundException(
          this.model.name,
          'Record not found for update operation.',
          {
            operation: 'update',
          },
        )
    ...
    const updated = await this.executeUpdateRow(
      uniqueWhere!,
      data as QuerySchemaUpdateData<TDelegate>,
    )
  ...
  public async updateOrInsert(
    ...
    values:
      ...
      | ((exists: boolean) => Record<string, unknown> | Promise<Record<string, unknown>>) = {},
  ): Promise<boolean> {
    if (
      typeof values !== 'function' &&
      this.adapter?.capabilities?.upsert &&
      typeof this.requireAdapter().upsert === 'function'
    ) {
      try {
  ...
  public async upsert(
    ...
    for (const row of values) {
      ...
      const updatePayload = (
        update ?? Object.keys(row).filter((key) => !uniqueKeys.includes(key))
  ...
  private normalizeInsertPayloads(
    ...
    return payloads.map((payload) => {
      ...
      if (primaryKeyValue === undefined || primaryKeyValue === null) {
        ...
        if (generated !== undefined) nextPayload[metadata.primaryKey] = generated
      }
  ...
  private normalizeUpdatePayload(
    ...
    for (const column of metadata.timestampColumns ?? []) {
      if (column.updatedAt) nextPayload[column.column] = now
    }
  ...
  private async resolveInsertUsingRows(
    ...
    const resolvedQuery =
      typeof query === 'function' ? await (query as () => unknown | Promise<unknown>)() : query
  ...
  private async resolveInsertUsingSource(source: unknown): Promise<Record<string, unknown>[]> {
    ...
    throw new QueryConstraintException(
      'insertUsing expects a query builder, array of records, or async resolver.',
      {
   ...
   * @returns
   */
  public async doesntExistOr<TResult>(
    callback: () => TResult | Promise<TResult>,
  ): Promise<boolean | TResult> {
   ...
   * @returns
   */
  public async sum<TKey extends keyof ModelAttributes<TModel> & string>(
    column: TKey,
  ): Promise<number> {
    const rows = (await this.executeReadRows()) as Record<string, unknown>[]
   ...
   * @returns
   */
  public async avg<TKey extends keyof ModelAttributes<TModel> & string>(
    column: TKey,
  ): Promise<number | null> {
    const rows = (await this.executeReadRows()) as Record<string, unknown>[]
    const values = rows
  ...
  public whereRaw(sql: string, bindings: unknown[] = []): this {
    if (!this.adapter?.capabilities?.rawWhere)
      throw new UnsupportedAdapterFeatureException(
        'Raw where clauses are not supported by the current adapter.',
        {
          operation: 'whereRaw',
          model: this.model.name,
          meta: {
  ...
  public async simplePaginate(
    ...
    const data = hasMorePages ? new ArkormCollection(items.all().slice(0, pageSize)) : items
   ...
   * @returns
   */
  private normalizeWith(relations: string | string[] | EagerLoadRelations<TModel>): EagerLoadMap {
    if (typeof relations === 'string') return { [relations]: undefined }
  ...
  private normalizeRelationAggregateInput(
    ...
    >((normalized, [name, value]) => {
      if (value === false || value === undefined) return normalized
  ...
  private normalizeQuerySelect(
    ...
    if (Array.isArray(select)) {
      if (select.some((expression) => typeof expression !== 'string')) return null
  ...
  private mergeRelationLoadPlans(
    ...
    const appendPlans = (plans?: RelationLoadPlan[]) => {
      plans?.forEach((plan) => {
        ...
        existing.relationLoads = this.mergeRelationLoadPlans(
          existing.relationLoads,
          plan.relationLoads,
        )
      })
    }
  ...
  private getRelationLoadSoftDeleteMode(): SoftDeleteQueryMode | undefined {
    if (this.onlyTrashedRecords) return 'only'

    if (this.includeTrashed) return 'include'
  ...
  public applyRelationLoadPlan(plan: RelationLoadPlan): this {
    if (plan.constraint) {
      ...
      if (!normalizedWhere) {
        throw new UnsupportedAdapterFeatureException(
          'Relation load plan constraints could not be normalized back into query where syntax.',
          {
    ...
    if (plan.orderBy) this.queryOrderBy = [...plan.orderBy]

    if (plan.columns) this.querySelect = [...plan.columns]

    this.queryDistinct = plan.distinct ?? false
    this.queryGroupBy = plan.groupBy ? [...plan.groupBy] : undefined

    if (plan.offset !== undefined) this.offsetValue = plan.offset

    if (plan.limit !== undefined) this.limitValue = plan.limit

    if (plan.relationLoads) this.with(this.relationLoadPlansToEagerLoadMap(plan.relationLoads))
  ...
  private tryBuildAdapterRelationLoadPlans(): RelationLoadPlan[] | null {
    ...
    for (const [path, constraint] of entries) {
      ...
      segments.forEach((segment, index) => {
        const existing = current.get(segment) ?? {
          ...
          children: new Map<string, RelationLoadTreeNode>(),
        }
    ...
    const toPlans = (
      owner: ModelStatic<any, any>,
      nodes: Map<string, RelationLoadTreeNode>,
    ): RelationLoadPlan[] | null => {
      const plans: RelationLoadPlan[] = []

      for (const [relation, node] of nodes.entries()) {
        const metadata = owner.getRelationMetadata(relation)
        const relatedModel =
          metadata && 'relatedModel' in metadata ? metadata.relatedModel : undefined
        ...
        const normalizedQuery =
          constrained instanceof QueryBuilder
        ...
        const where = normalizedQuery.legacyWhere
          ? normalizedQuery.tryBuildQueryCondition(
              normalizedQuery.legacyWhere as QuerySchemaWhere<any>,
            )
  ...
  private async eagerLoadModels(models: TModel[]): Promise<void> {
    ...
    if (
      adapter?.capabilities?.relationLoads === true &&
      typeof adapter.loadRelations === 'function' &&
      relationLoads !== null
    ) {
  ...
  private normalizeRelationLoads(include: unknown): RelationLoadPlan[] | null {
    ...
    for (const [relation, value] of Object.entries(include as Record<string, unknown>)) {
      ...
      const constraint =
        options.where === undefined ? undefined : this.tryBuildQueryCondition(options.where)
      const orderBy =
        options.orderBy === undefined
          ? undefined
          : this.normalizeRelationLoadOrderBy(options.orderBy)
      const columns =
        options.select === undefined ? undefined : this.normalizeRelationLoadSelect(options.select)
      const relationLoads =
        options.include === undefined ? undefined : this.normalizeRelationLoads(options.include)

      if (constraint === null || orderBy === null || columns === null || relationLoads === null)
        return null

      if (
        (options.skip !== undefined && typeof options.skip !== 'number') ||
  ...
  private toQuerySchemaWhere(condition?: QueryCondition): QuerySchemaWhere<TDelegate> | undefined {
    ...
    if (condition.type === 'comparison') {
      ...
      if (condition.operator === 'not-in')
        return {
          [condition.column]: {
            notIn: Array.isArray(condition.value) ? condition.value : [condition.value],
          },
        } as QuerySchemaWhere<TDelegate>
      ...
      if (condition.operator === 'starts-with')
        return {
          [condition.column]: { startsWith: condition.value },
        } as QuerySchemaWhere<TDelegate>
  ...
  private buildSoftDeleteQueryCondition(): QueryCondition | undefined {
    ...
    if (!softDeleteConfig.enabled || this.includeTrashed) return undefined
  ...
  private tryBuildFieldCondition(column: string, value: unknown): QueryCondition | null {
    if (value === null) return { type: 'comparison', column, operator: 'is-null' }
      ...
      if (key === 'NOT') {
        const nested = Array.isArray(value)
          ? value
              .map((entry) => this.tryBuildQueryCondition(entry))
              .filter((entry): entry is QueryCondition => entry !== undefined)
          : [this.tryBuildQueryCondition(value)].filter(
              (entry): entry is QueryCondition => entry !== undefined,
            )
  ...
  private tryBuildSelectSpec(
    ...
    if (
      ...
      this.canExecuteRelationFiltersInAdapter() &&
      relationFilters === null
    )
      return null

    if (
      this.hasRelationAggregates() &&
      this.canExecuteRelationAggregatesInAdapter() &&
      relationAggregates === null
    )
      return null
  ...
  private async executeReadRows(
    ...
    const adapter = this.requireAdapter()
    const spec = this.tryBuildSelectSpec(
      useWhereOverride ? whereOverride : this.buildWhere(),
      useWhereOverride,
    )
    if (!spec)
      throw new UnsupportedAdapterFeatureException(
  ...
  private async executeReadExists(): Promise<boolean> {
    ...
    if (typeof adapter.exists === 'function') return await adapter.exists({ ...spec, limit: 1 })
  ...
  private async executeUpsertRows(
    ...
    const adapter = this.requireAdapter()
    const payloads = this.normalizeInsertPayloads(
      values as QuerySchemaCreateData<TDelegate>[],
    ...
    if (typeof adapter.upsert !== 'function') {
      throw new UnsupportedAdapterFeatureException(
        'Upsert is not supported by the current adapter.',
        {
    ...
    return await adapter.upsert(
      this.tryBuildUpsertSpec(payloads, uniqueBy, normalizedUpdateColumns),
    )
  }
  ...
  private async executeUpdateRow(
    ...
    if (!spec)
      throw new UnsupportedAdapterFeatureException(
        'Update could not be compiled into an Arkorm update specification.',
        {
  ...
  private async executeUpdateManyRows(
    ...
    if (!spec)
      throw new UnsupportedAdapterFeatureException(
        'Update-many could not be compiled into an Arkorm update specification.',
        {
  ...
  private buildWhere(): QuerySchemaWhere<TDelegate> | undefined {
    ...
    if (!baseWhere) return softDeleteClause as QuerySchemaWhere<TDelegate>
  ...
  private async resolveUniqueWhere(
    ...
    if (!condition)
      throw new UniqueConstraintResolutionException(
        'Unable to resolve a unique identifier for update/delete operation from the current query shape.',
        {
    ...
    if (!Object.prototype.hasOwnProperty.call(row, primaryKey))
      throw new UniqueConstraintResolutionException(
        `Unable to resolve a unique identifier for update/delete operation. Include [${primaryKey}] in the query constraints.`,
        {
          operation: 'resolveUniqueWhere',
          model: this.model.name,
          meta: {
  ...
  private isUniqueWhere(where: Record<string, unknown>): boolean {
    ...
    return (
      Object.keys(where).length === 1 && Object.prototype.hasOwnProperty.call(where, primaryKey)
    )
  }
  ...
  private canExecuteRelationFiltersInAdapter(): boolean {
    ...
    return (
      adapter?.capabilities?.relationFilters === true && this.tryBuildRelationFilterSpecs() !== null
    )
  }
  ...
  private canExecuteRelationAggregatesInAdapter(): boolean {
    ...
    return (
      adapter?.capabilities?.relationAggregates === true &&
  ...
  private canExecuteRelationFeaturesInAdapter(): boolean {
    ...
    const aggregatesSupported =
      !this.hasRelationAggregates() || this.canExecuteRelationAggregatesInAdapter()

    return filtersSupported && aggregatesSupported
  }

  private shouldUseCompatibilityRelationFallback(
    useAdapterRelationFeatures = this.canExecuteRelationFeaturesInAdapter(),
  ): boolean {
    ...
    if (sqlRelationFiltersRejected || sqlRelationAggregatesRejected) {
      throw new UnsupportedAdapterFeatureException(
        'Relation filters or aggregates could not be compiled into Arkorm adapter specifications for the current query shape.',
        {
  ...
  private hasUncompilableSqlRelationFilters(adapter?: DatabaseAdapter): boolean {
    if (!this.hasRelationFilters() || adapter?.capabilities?.relationFilters !== true) return false
  ...
  private isSqlRelationFeatureMetadata(
    metadata: ReturnType<ModelStatic<TModel, TDelegate>['getRelationMetadata']>,
  ): boolean {
  ...
  private buildSoftDeleteOnlyWhere(): QuerySchemaWhere<TDelegate> | undefined {
    const softDeleteConfig = this.model.getSoftDeleteConfig()
    if (!softDeleteConfig.enabled) return undefined

    if (this.includeTrashed) return undefined
  ...
  private async applyRelationAggregates(
    ...
    const cache =
      relationCache ?? new WeakMap<object, Map<string, Map<unknown, Promise<RelationResult>>>>()
    await Promise.all(
      models.map(async (model) => {
        for (const aggregate of this.relationAggregates) {
          const results = await this.resolveRelatedResults(
            model,
            aggregate.relation,
            cache,
            aggregate.callback,
          ...
          const values = list
            .map((item) =>
              (item as { getAttribute: (key: string) => unknown }).getAttribute(
                aggregate.column as string,
              ),
            )
            ...
            const min =
              values.length === 0
                ...
                : values.reduce((left, right) =>
                    (right as number | string | Date) < (left as number | string | Date)
                      ? right
                      : left,
                  )
    ...
    const resolver = (async () => {
      ...
      if (typeof relationMethod !== 'function')
        throw new RelationResolutionException(
          `Relation [${relation}] is not defined on the model.`,
          {
      ...
      const relationInstance = relationMethod.call(model) as {
        constrain?: (
          constraint: (query: QueryBuilder<any, any>) => QueryBuilder<any, any> | void,
        ) => unknown
        get?: () => Promise<unknown>
      ...
      throw new RelationResolutionException(
        `Relation [${relation}] does not support result resolution.`,
        {
          operation: 'resolveRelatedResults',
          model: this.model.name,
          relation,
