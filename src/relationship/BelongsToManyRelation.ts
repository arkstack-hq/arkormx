...
export class BelongsToManyRelation<TParent, TRelated> extends Relation<TRelated> {
  ...
  public wherePivot(column: string, operatorOrValue: unknown, value?: unknown): this {
    ...
    if (arguments.length === 2)
      return this.addPivotWhere(this.makePivotComparison(normalizedColumn, '=', operatorOrValue))

    return this.addPivotWhere(
      this.makePivotComparison(normalizedColumn, operatorOrValue as QueryComparisonOperator, value),
    )
  }
  ...
  private resolveRelatedPivotValue(related: TRelated | unknown): unknown {
    if (
      ...
      'getAttribute' in (related as Record<string, unknown>)
    ) {
  ...
  public async sync(
    ...
    return await adapter.transaction(async (transaction) => {
      ...
      const desiredEntries = new Map<
        string,
        { related: TRelated | unknown; attributes: Record<string, unknown> }
      >()
      ...
      for (const [relatedKey, entry] of desiredEntries) {
        if (!existingKeys.has(relatedKey)) {
          await this.insertPivotRow(
            this.buildPivotInsertValues(entry.related, entry.attributes),
            transaction,
          )
          attached += 1
  ...
  private getPivotColumnSelection(): string[] {
    return [this.foreignPivotKey, this.relatedPivotKey, ...this.pivotColumns].filter(
  ...
  private createPivotRecord(row: Record<string, unknown>): unknown {
    const attributes = this.getPivotColumnSelection().reduce<Record<string, unknown>>(
      (all, column) => {
  ...
  private attachPivotToResults(
    ...
    results.all().forEach((related) => {
      const model = related as unknown as {
  ...
  private decorateQueryBuilder(
    ...
    decorated.paginate = (async (perPage = 15, page?: number, options = {}) => {
      ...
      return new LengthAwarePaginator(
        ...
        paginator.meta.perPage,
        paginator.meta.currentPage,
        options,
      )
