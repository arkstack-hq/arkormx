...
export class DB {
  ...
  public async raw<TRow extends Record<string, unknown> = Record<string, unknown>>(
    ...
    if (!adapter.rawQuery)
      throw new UnsupportedAdapterFeatureException(
        'Raw queries are not supported by the current adapter.',
        {
          operation: 'db.raw',
          meta: {
            feature: 'rawQuery',
  ...
  private static createTableModel<TRow extends Record<string, unknown>>(
    ...
    const timestampColumns =
      options.timestampColumns?.map((column) => ({ ...column })) ??
    ...
    const modelStatic = {
      query: (): QueryBuilder<TRow, ModelQuerySchemaLike> =>
        new QueryBuilder<TRow, ModelQuerySchemaLike>(
      ...
      hydrateMany: (attributes: Record<string, unknown>[]): TRow[] =>
        attributes.map((attribute) => ({ ...attribute }) as TRow),
      hydrateRetrieved: async (attributes: Record<string, unknown>): Promise<TRow> =>
        ({ ...attributes }) as TRow,
      hydrateManyRetrieved: async (attributes: Record<string, unknown>[]): Promise<TRow[]> =>
        attributes.map((attribute) => ({ ...attribute }) as TRow),
  ...
  private static resolvePersistedTableMetadata(
    ...
    if (options.persistedMetadata === false) return { columns: {}, enums: {} }

    const persistedMetadataOptions =
      typeof options.persistedMetadata === 'object' ? options.persistedMetadata : {}
    ...
    return getPersistedTableMetadata(table, {
      ...
      strict:
        persistedMetadataOptions.strict ??
