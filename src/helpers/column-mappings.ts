...
export const resolveColumnMappingsFilePath = (cwd: string, configuredPath?: string): string => {
...
const normalizePersistedTableMetadata = (table: unknown): PersistedTableMetadata => {
  if (!table || typeof table !== 'object' || Array.isArray(table)) return { columns: {}, enums: {} }
  ...
  const enums = Object.entries(candidate.enums ?? {}).reduce<Record<string, string[]>>(
...
const normalizePersistedPrimaryKeyGeneration = (
  value: unknown,
  ...
  return {
    ...
    strategy: 'uuid',
    prismaDefault:
      typeof candidate.prismaDefault === 'string' && candidate.prismaDefault.trim().length > 0
        ? candidate.prismaDefault
        : undefined,
    databaseDefault:
      typeof candidate.databaseDefault === 'string' && candidate.databaseDefault.trim().length > 0
        ? candidate.databaseDefault
        : undefined,
    runtimeFactory: candidate.runtimeFactory === 'uuid' ? 'uuid' : undefined,
  }
}

const normalizePersistedTimestampColumns = (
  value: unknown,
): PersistedTimestampColumn[] | undefined => {
...
const normalizePersistedColumnMappingsState = (
  ...
  const tables = Object.entries(state?.tables ?? {}).reduce<Record<string, PersistedTableMetadata>>(
    (all, [tableName, tableMetadata]) => {
      if (tableName.trim().length === 0) return all

      const normalized = normalizePersistedTableMetadata(tableMetadata)
      if (
        Object.keys(normalized.columns).length > 0 ||
        Object.keys(normalized.enums).length > 0 ||
        normalized.primaryKeyGeneration ||
        normalized.timestampColumns?.length
      )
...
const buildPersistedFeatureDisabledError = (
  ...
  const label =
    feature === 'persistedColumnMappings' ? 'persisted column mappings' : 'persisted enum metadata'
  const configKey =
    feature === 'persistedColumnMappings'
      ? 'features.persistedColumnMappings'
      : 'features.persistedEnums'

  return new ArkormException(
    `Table [${table}] requires ${label}, but ${configKey} is disabled in arkormx.config.*.`,
    {
...
export const readPersistedColumnMappingsState = (
  ...
  try {
    ...
    ) as Partial<PersistedColumnMappingsState>
...
export const deletePersistedColumnMappingsState = (filePath: string): void => {
...
export const getPersistedTableMetadata = (
  ...
  const state = readPersistedColumnMappingsState(
    resolveColumnMappingsFilePath(options.cwd ?? process.cwd(), options.configuredPath),
  )
  ...
  return {
    ...
    enums: Object.entries(metadata.enums).reduce<Record<string, string[]>>(
      (all, [columnName, values]) => {
        all[columnName] = [...values]
...
const applyMappedColumn = (
  ...
  if (
    typeof column.map === 'string' &&
    column.map.trim().length > 0 &&
...
const removePersistedColumnMetadata = (
  ...
  if (tableMetadata.timestampColumns) {
    tableMetadata.timestampColumns = tableMetadata.timestampColumns.filter(
...
export const applyOperationsToPersistedColumnMappingsState = (
  ...
  const nextTables = Object.entries(state.tables).reduce<Record<string, PersistedTableMetadata>>(
    (all, [table, metadata]) => {
      all[table] = {
        columns: { ...metadata.columns },
        enums: Object.entries(metadata.enums).reduce<Record<string, string[]>>(
  ...
  operations.forEach((operation) => {
    if (operation.type === 'createTable') {
      ...
      if (
        Object.keys(tableMetadata.columns).length > 0 ||
        Object.keys(tableMetadata.enums).length > 0 ||
        tableMetadata.primaryKeyGeneration ||
        tableMetadata.timestampColumns?.length
      )
...
export const rebuildPersistedColumnMappingsState = async (
  ...
  const availableByIdentity = new Map<string, MigrationClass>(
    availableMigrations.map(([migrationClass, file]) => [
      buildMigrationIdentity(file, migrationClass.name),
      migrationClass,
    ]),
  )
