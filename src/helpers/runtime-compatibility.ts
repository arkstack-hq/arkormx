...
const getCompatibilitySources = (
  preferredClient?: RuntimeClientLike,
): Array<RuntimeClientLike | undefined> => {
...
export const getRuntimeCompatibilityAdapter = (
  preferredClient?: RuntimeClientLike,
): DatabaseAdapter | undefined => {
  const client = getCompatibilitySources(preferredClient).find((source) =>
    isCompatibilityClient(source),
  )
...
export const resolveRuntimeCompatibilityQuerySchemaOrThrow = <
  TSchema extends ModelQuerySchemaLike = ModelQuerySchemaLike,
>(
  key: string,
  candidates: string[],
  modelName: string,
