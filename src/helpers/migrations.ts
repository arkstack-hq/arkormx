import {
  ...
  PrismaMigrationWorkflowOptions,
  PrismaSchemaSyncOptions,
  SchemaColumn,
  SchemaForeignKey,
...
export const toModelName = (tableName: string): string => {
  ...
  const singular =
    normalized.endsWith('s') && normalized.length > 1 ? normalized.slice(0, -1) : normalized
  ...
  for (const value of normalizedValues) {
    if (seen.has(value)) {
      throw new ArkormException(
        `Prisma enum [${enumName}] for column [${column.name}] contains duplicate value [${value}].`,
      )
    }
 ...
 * @returns
 */
const validateEnumDefaultValue = (
  column: SchemaColumn,
  enumName: string,
  enumValues: string[],
): void => {
  ...
  throw new ArkormException(
    `Enum default value [${normalizedDefault}] is not defined in Prisma enum [${enumName}] for column [${column.name}].`,
  )
}
...
export const buildFieldLine = (column: SchemaColumn): string => {
  if (column.type === 'id') {
    ...
    const mapped =
      typeof column.map === 'string' && column.map.trim().length > 0
...
export const findEnumBlock = (
  ...
  for (const match of candidates) {
    ...
    if (matchedEnumName === enumName) return { enumName: matchedEnumName, block, start, end }
  }
...
const ensureEnumBlocks = (schema: string, columns: SchemaColumn[]): string => {
  ...
  for (const column of columns) {
    ...
    if (existing) {
      const existingValues = validateEnumValues(
        column,
        enumName,
        extractEnumBlockValues(existing.block),
      )
    ...
    if (enumValues.length === 0) {
      throw new ArkormException(
        `Prisma enum [${enumName}] was not found for column [${column.name}].`,
      )
    }
...
export const buildPrimaryKeyLine = (primaryKey: SchemaPrimaryKey): string => {
  ...
  const named =
    typeof primaryKey.name === 'string' && primaryKey.name.trim().length > 0
...
export const buildUniqueConstraintLine = (constraint: SchemaUniqueConstraint): string => {
  ...
  const named =
    typeof constraint.name === 'string' && constraint.name.trim().length > 0
...
export const buildRelationLine = (
  ...
  const relationName = deriveRelationAlias(
    sourceModelName,
    targetModel,
    foreignKey.relationAlias?.trim(),
  )
...
const injectLineIntoModelBody = (
  ...
  if (alreadyExists) return bodyLines
...
const applyInverseRelations = (
  ...
  for (const foreignKey of foreignKeys) {
    ...
    const sourceColumn = resolveForeignKeyColumn(columns, foreignKey)
    const inverseLine = buildInverseRelationLine(
      sourceModelName,
...
export const buildModelBlock = (operation: SchemaTableCreateOperation): string => {
  ...
  const relations = (operation.foreignKeys ?? []).map((foreignKey) =>
    buildRelationLine(modelName, foreignKey, operation.columns),
  )
  ...
  const lines =
    metadata.length > 0 ? [...fields, ...relations, '', ...metadata] : [...fields, ...relations]
...
export const findModelBlock = (
  ...
  for (const match of candidates) {
    ...
    const start = match.index ?? 0
    const end = start + block.length
    if (explicitMapRegex.test(block)) return { modelName, block, start, end }

    if (modelName.toLowerCase() === table.toLowerCase()) return { modelName, block, start, end }
 ...
 */
export const applyCreateTableOperation = (
  schema: string,
  operation: SchemaTableCreateOperation,
): string => {
...
export const applyAlterTableOperation = (
  ...
  operation.addColumns.forEach((column) => {
    ...
    const afterInsertIndex =
      typeof column.after === 'string' && column.after.length > 0
        ? bodyLines.findIndex((line) =>
            new RegExp(`^\\s*${escapeRegex(column.after as string)}\\s+`).test(line),
          )
        : -1
    const insertIndex =
  ...
  if (operation.addPrimaryKey) {
    ...
    const hasPrimaryKey = bodyLines.some(
      (line) => line.includes(' @id') || line.trim().startsWith('@@id('),
    )
    if (hasPrimaryKey)
      throw new ArkormException(
        `Prisma model for table [${operation.table}] already defines a primary key.`,
      )
  ...
  for (const foreignKey of operation.addForeignKeys ?? []) {
    ...
    const relationRegex = new RegExp(
      `^\\s*${escapeRegex(foreignKey.fieldAlias?.trim() || deriveRelationFieldName(foreignKey.column))}\\s+`,
    )
 ...
 */
export const applyOperationsToPrismaSchema = (
  schema: string,
 ...
 * @returns void
 */
export const runPrismaCommand = (args: string[], cwd: string): void => {
 ...
 * @returns
 */
export const buildMigrationSource = (className: string, extension: 'ts' | 'js' = 'ts'): string => {
  if (extension === 'js') {
    return [
