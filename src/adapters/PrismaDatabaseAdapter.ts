...
export class PrismaDatabaseAdapter implements DatabaseAdapter {
  ...
  public constructor(
    ...
    this.delegates = Object.entries(prisma).reduce<Record<string, PrismaDelegateLike>>(
    ...
    this.capabilities = {
      ...
      insertMany: Object.values(this.delegates).some(
        (delegate) => typeof (delegate as { createMany?: unknown }).createMany === 'function',
      ),
      upsert: false,
      updateMany: Object.values(this.delegates).some(
        (delegate) => typeof (delegate as { updateMany?: unknown }).updateMany === 'function',
      ),
      deleteMany: false,
      exists: true,
      relationLoads: false,
  ...
  private toQuerySelect(columns?: QuerySelectColumn[]): PrismaLikeSelect | undefined {
    ...
    if (rawColumn) {
      throw new UnsupportedAdapterFeatureException(
        'Raw select expressions are not supported by the Prisma compatibility adapter; use a SQL-backed adapter or DB.raw().',
        {
    ...
    if (columns.some((column) => column.wildcard)) return undefined
  ...
  private toQueryWhere(condition?: QueryCondition): PrismaLikeWhereInput | undefined {
    ...
    if (condition.type === 'raw') {
      throw new UnsupportedAdapterFeatureException(
        'Raw where clauses are not supported by the Prisma compatibility adapter; use a SQL-backed adapter for raw SQL predicates.',
        {
    ...
    throw new UnsupportedAdapterFeatureException(
      `Where condition [${condition.type}] is not supported by the Prisma compatibility adapter; use a SQL-backed adapter.`,
      {
        operation: 'adapter.where',
        meta: {
  ...
  private buildFindArgs(spec: SelectSpec<any>): {
    ...
    if (spec.distinct) {
      throw new UnsupportedAdapterFeatureException(
        'Distinct queries are not supported by the Prisma compatibility adapter.',
        {
          operation: 'adapter.select',
    ...
    if (spec.groupBy?.length) {
      throw new UnsupportedAdapterFeatureException(
        'Group-by queries are not supported by the Prisma compatibility adapter.',
        {
          operation: 'adapter.select',
    ...
    if (spec.having) {
      throw new UnsupportedAdapterFeatureException(
        'Having clauses are not supported by the Prisma compatibility adapter; use a SQL-backed adapter.',
        {
          operation: 'adapter.select',
          meta: { feature: 'having' },
        },
    ...
    if (spec.joins?.length) {
      throw new UnsupportedAdapterFeatureException(
        'Join clauses are not supported by the Prisma compatibility adapter; use a SQL-backed adapter or DB.raw().',
        {
          operation: 'adapter.select',
          meta: { feature: 'joins' },
        },
  ...
  private toQueryInclude(relationLoads?: RelationLoadPlan[]): PrismaLikeInclude | undefined {
    if (!relationLoads || relationLoads.length === 0) return undefined
      ...
      return [
        {
          ...
          fields: (model.fields ?? [])
            ...
            .map((field) => {
              const enumValues =
                field.kind === 'enum'
                  ? ((enums[field.type]?.values ?? []) as Array<string | { name?: string }>)
  ...
  private resolveDelegate(target: QueryTarget<any>): PrismaDelegateLike {
    ...
    throw new MissingDelegateException(
      'Prisma delegate could not be resolved for adapter target.',
      {
  ...
  public async select<TModel = unknown>(spec: SelectSpec<TModel>): Promise<DatabaseRow[]> {
    ...
    return await this.runWithDebug(
      'select',
      spec.target,
      async () => {
  ...
  public async insertMany<TModel = unknown>(spec: InsertManySpec<TModel>): Promise<number> {
    ...
    if (typeof delegate.createMany === 'function') {
      const result = await this.runWithDebug(
        'insertMany',
        spec.target,
        async () => {
  ...
  public async deleteMany<TModel = unknown>(spec: DeleteManySpec<TModel>): Promise<number> {
    const delegate = this.resolveDelegate(spec.target)
    const where = this.toQueryWhere(spec.where)
    const rows = await this.runWithDebug(
      'deleteMany',
      spec.target,
      async () => {
  ...
  public async loadRelations<TModel = unknown>(_spec: RelationLoadSpec<TModel>): Promise<void> {
    throw new UnsupportedAdapterFeatureException(
      "Adapter-owned relation batch loading is intentionally unavailable on the Prisma compatibility adapter; eager loading stays on Arkorm's generic loader for this path.",
      {
  ...
  public async transaction<TResult = unknown>(
    ...
    if (!this.hasTransactionSupport(this.prisma)) {
      throw new UnsupportedAdapterFeatureException(
        'Transactions are not supported by the Prisma compatibility adapter.',
        {
    ...
    return await this.prisma.$transaction(
      async (transactionClient: PrismaClientLike) => {
