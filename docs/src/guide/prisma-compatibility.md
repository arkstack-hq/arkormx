...
- the SQL join family (`join(...)`, `leftJoin(...)`, `joinSub(...)`, lateral joins, and the rest) does not work on the compatibility adapter; use a SQL-backed adapter such as Kysely.
